...
// ===========================================================================
// Random cuFloatComplex in [0,1] + i[0,1]
// ===========================================================================
static inline cuFloatComplex crand(void) {
...
// ===========================================================================
static void pack_block(const cuFloatComplex *dense,
                       int n, int b,
                       int r0, int c0,
                       int k,
                       cuFloatComplex *data)
{
    cuFloatComplex *dst = data + (size_t)k * (size_t)b * (size_t)b;
    for (int j = 0; j < b; ++j) {
        for (int i = 0; i < b; ++i) {
...
static void pack_block_rect(const cuFloatComplex *dense,
                            int n,
                            int r0, int c0,
                            int rlen, int clen,
                            size_t dst_offset,
                            cuFloatComplex *data)
{
    cuFloatComplex *dst = data + dst_offset;
...
int create_test_matrix(int n, int b, int block_structure,
                       cuFloatComplex *dense, block_sparse_format *bsf)
{
    #define NUM_BLOCKS 8
    ...
    if (block_structure == 0) {
        ...
        cuFloatComplex *data = (cuFloatComplex*)
        ...
        cuFloatComplex *data = (cuFloatComplex*)malloc(total_entries * sizeof *data);
        if (!data) { fprintf(stderr, "Allocation failed for data buffer\n"); free(block_lengths); return 1; }
    ...
    fprintf(stderr, "block_structure %d not implemented in this snippet\n", block_structure);
    return -1;
}
...
// ===========================================================================
void print_test_matrix_information(int n, cuFloatComplex *dense, block_sparse_format *bsf) {
    // Print dense matrix
    printf("\nDense matrix (row-major):\n");
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++)
            printf("(%5.2f,%5.2f) ", cuCrealf(dense[i*n + j]), cuCimagf(dense[i*n + j]));
        printf("\n");
    }
    
    // Print information about the sparse matrix
    printf("Global size: %d x %d\n", bsf->n, bsf->n);
    for (int r = 0; r < bsf->num_slices; r++)
        printf("Block row %d: rows %d to %d\n", r, bsf->slice_ranges[r].start, bsf->slice_ranges[r].end);
    ...
    for (int bidx = 0; bidx < bsf->num_blocks; bidx++) {
        ...
        int row_start = bsf->slice_ranges[row].start;
        int row_end   = bsf->slice_ranges[row].end;
        int col_start = bsf->slice_ranges[col].start;
        int col_end   = bsf->slice_ranges[col].end;
        int rows = row_end - row_start + 1;
        int cols = col_end - col_start + 1;

        printf("\nBlock %d at (%d,%d), size %d x %d:\n",
            bidx, row, col, rows, cols);
        cuFloatComplex *blk_data = bsf->flatData + (size_t)bsf->offsets[bidx];
        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < cols; j++) {
                printf("(%5.2f,%5.2f) ", cuCrealf(blk_data[(size_t)j*(size_t)rows + (size_t)i]),
                                       cuCimagf(blk_data[(size_t)j*(size_t)rows + (size_t)i]));
            }
            printf("\n");
        }
    }
